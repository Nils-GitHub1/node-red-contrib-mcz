...
const COMMANDS = {
    ...
    "Temperature":      { id: 42,   type: "temperature", min: 5, max: 40, unit: "°C" },
    "Boiler_Setpoint":  { id: 51,   type: "temperature", min: 5, max: 80, unit: "°C" },
    "Power_Level":      { id: 36,   type: "int", min: 1, max: 5 },
    "Fan_State":        { id: 37,   type: "int", min: 0, max: 6 },
    "DuctedFan1":       { id: 38,   type: "int", min: 0, max: 6 },
    "DuctedFan2":       { id: 39,   type: "int", min: 0, max: 6 },
...
function buildCommandString(name, rawValue) {
    ...
    if (cmd.min !== undefined && v_num < cmd.min) {
        return null;
    }

    if (cmd.max !== undefined && v_num > cmd.max) {
        return null;
    }
...
/**
 * Build a raw WriteParametri command.
 *
 * Used for undocumented or future parameters.
 *
 * Example: buildRawWrite(42,43)
 *
 * returns: C|WriteParametri|42|43
 */
function buildRawWrite(id, value) {
    if (id === undefined || value === undefined) {
        return null;
    }

    return `C|WriteParametri|${id}|${value}`;
}

/**
 * Build a raw Diagnostica command.
 *
 * Example: buildDiagnostic(8,75)
 *
 * returns: C|Diagnostica|8|75
 */
function buildDiagnostic(id,value) {
    if (id === undefined || value === undefined) {
        return null;
    }

    return `C|Diagnostica|${id}|${value}`;
}
